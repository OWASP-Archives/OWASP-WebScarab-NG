...
public class JdbcConversationDao extends PropertiesJdbcDaoSupport implements
	...
	private class ConversationSummaryQuery extends MappingSqlQuery {
		...
		public ConversationSummaryQuery() {
			super(
					...
					"SELECT "
							...
							+ "request_content_checksum, request_content_size, response_version, "
							+ "response_status, messages.message as response_message, response_content_checksum, response_content_size, plugin "
		...
		int rownum) throws SQLException {
			...
			summary.setRequestContentSize(rs.getInt("request_content_size"));
			...
			summary.setResponseContentSize(rs.getInt("response_content_size"));
			return summary;
		}
	}
	...
	private class ConversationSummaryInsert extends SqlUpdate {
		...
		protected ConversationSummaryInsert() {
			super(
					...
					"INSERT INTO conversations ("
							+ "session, date, request_method, request_uri, request_version, request_content_checksum, request_content_size, "
							+ "response_version, response_status, response_message, response_content_checksum, response_content_size, plugin) "
							+ "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
			declareParameter(new SqlParameter(Types.INTEGER));
		...
		protected Integer insert(Integer session, ConversationSummary summary) {
			...
			Object[] objs = new Object[] { session, summary.getDate(),
					...
					summary.getResponseContentSize(),
